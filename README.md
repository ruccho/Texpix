...
- **Components batch by font, not by style.** Outline color and mode travel in the vertex stream, so every `Texpix Text` on the default material shares one material instance and uGUI batches them regardless of their outline settings. What still splits a batch is a different atlas texture: a different font asset, a fallback renderer, or an inline-sprite renderer. Assigning a custom material to some components splits them off as well.
